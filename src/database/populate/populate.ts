...
// @ts-ignore
logger.log('? envVar:', envVar, ' /__dirname:', __dirname, ' /INIT_CWD:', process.env.INIT_CWD, ' /require.main.filename:', require && require.main && require.main.filename);
...
async function getSourceData(): Promise<TPopulatedData | TPopulatedData[]> {
    ...
    if (!sourceDataPath) {
        return Promise.reject(
            `Path to data was not provided! You must pass "${PARAMS.PRODUCTS}" parameter.`
        );
    }
...
async function populateProducts(ProductModel: TGenericModel, sourceDataList: TPopulatedData | TPopulatedData[]) {
    ...
    function categoryNameGrouper(): TCategoryNameGrouper {
        ...
        const categoryRegExps: CustomRegExp[] = JSON.parse(
            readFileSync(categoriesGroupPath, { encoding: 'utf8' })
        );
...
async function assignIDsToRelatedProducts(Product: TGenericModel) {
    ...
    logger.log(
        'All done?', amountOfAllProducts === amountOfRelatedProductsWithID,
        ' /amountOfAllProducts:', amountOfAllProducts,
        ' /amountOfRelatedProductsWithID:', amountOfRelatedProductsWithID
    );
}
