...
export function getOrderPaymentMethod(token: string, minPrice: number, maxPrice: number): Promise<Partial<IPayByLinkMethod>> {
    const PAYU_METHODS_URL: string = 'https://secure.snd.payu.com/api/v2_1/paymethods';
    ...
    function filterPaymentMethods(methods: IPayByLinkMethod[]): IPayByLinkMethod[] {
        return methods.filter((method) => {
            return method.status === 'ENABLED' && method.minAmount <= minPrice && method.maxAmount >= maxPrice &&
                !EXCLUDED_METHODS.find((excludedMethod) =>
                    excludedMethod.value === method.value && excludedMethod.name === method.name
                );
        });
    }
