...
async function saveUserRole(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!req.body) {
      ...
      return res.status(HTTP_STATUS_CODE.BAD_REQUEST).json({ error: 'Param "permissions" is empty or not attached!' });
    }
    ...
    return res.status(HTTP_STATUS_CODE.OK).json({ payload: savedUserRole });
  } catch (exception) {
...
async function updateUserRole(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    return res.status(HTTP_STATUS_CODE.OK).json({ payload: updatedUserRole });
  } catch (exception) {
    return next(exception);
  }
}
...
async function getUserRole(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    return res.status(HTTP_STATUS_CODE.OK).json({ payload: userRole });
  } catch (exception) {
    return next(exception);
  }
}
