...
(function UserCommands() {
    const userAPIReq = (urlSuffix, method, payload, canFail = true) => {
        if (!urlSuffix || !method || !payload) {
            throw ReferenceError(`'urlSuffix', 'method' and 'payload' must be defined! 
    ...
    Cypress.Commands.add('confirmTestUserRegistrationByUI', (email) => {
        return cy.getLinkFromEmail(email, 'Account activation', '/pages/confirm-registration').then((link) => {
            ...
            cy.contains('[data-cy="message:registration-confirmation-succeeded-hint"]', 'You can now log in to your new account.');
        });
    });
    ...
    Cypress.Commands.add('confirmTestUserRegistration', (email) => {
        return cy
            ...
            .then((link) => {
            const token = new URLSearchParams(link.search).get('token').replace(/\s/g, '+');
    ...
    Cypress.Commands.add('removeTestUsers', (canFail) => {
        return userAPIReq('delete', 'DELETE', {
            queryType: 'regex',
            rawQuery: 'test user',
        }, canFail);
    });
